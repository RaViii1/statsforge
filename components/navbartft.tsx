...
export default function NavbarTft() {
  ...
  const performSearch = async (gameName: string, tagLine: string, server: string) => {
    ...
    try {
      ...
      if (response.status === 404) {
        router.push(`/tft/profile/player-not-found?name=${encodeURIComponent(gameName)}&tag=${encodeURIComponent(tagLine)}&server=${server}`);
        setMobileMenuOpen(false);
        return;
      }

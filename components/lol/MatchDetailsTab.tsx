  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="relative inline-flex items-stretch rounded-2xl bg-zinc-900/60 backdrop-blur-md border border-zinc-800/80 p-1.5 shadow-2xl shadow-black/20">
          {/* Sliding background indicator */}
          <div
            className="absolute top-1.5 bottom-1.5 rounded-xl bg-linear-to-br from-orange-500 to-orange-600 shadow-lg shadow-orange-500/25 transition-all duration-300 ease-out"
            style={{
              left: arena ? '3px' : `${tabs.findIndex(t => t.id === activeTab) * (100 / tabs.length)}%`,
              width: arena ? 'calc(100% - 6px)' : `calc(${100 / tabs.length}% - 6px)`,
              marginLeft: arena ? '0' : '3px',
            }}
          />
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`
                  relative z-10 flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-xl
                  font-semibold text-sm whitespace-nowrap transition-all duration-300 ease-out
                  min-w-[140px] sm:min-w-40
                  ${isActive 
                    ? 'text-white' 
                ...
                `}
              >
                <Icon className={`w-4 h-4 transition-all duration-300 ${isActive ? 'scale-110' : 'scale-100'}`} />
        ...
        <button
          ...
          className="relative inline-flex items-center gap-2.5 px-6 py-3.5 rounded-xl font-semibold text-sm bg-linear-to-br from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-lg shadow-orange-500/25 hover:shadow-xl hover:shadow-orange-500/40 transition-all duration-300 hover:scale-105 border border-orange-400/20"
        >
                    ...
                    <div className="space-y-3">
                      {players.map((participant) => {
                        ...
                        return (
                          ...
                          <span className="text-[9px] text-white absolute bottom-0 left-0 bg-zinc-900 rounded p-0.5 ">{participant.champLevel}</span>
                              <img
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-3">
                      {/* Champion + Spells + Runes + Rank */}
                      <div className="flex items-center gap-0.5 overflow-visible shrink-0">
                        <div className="relative group pr-1.5">
                          <span className="text-[9px] text-white absolute bottom-0 left-0 bg-zinc-900 rounded p-0.5 ">{participant.champLevel}</span>
                          <img
                            src={`https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champion-icons/${participant.championId}.png`}
                            alt={participant.championName}
                            className="w-14 h-14 rounded-lg border border-zinc-600 group-hover:border-orange-500 shadow-md transition-all"
                            title={participant.championName}
                          />
                        </div>
                        <div className="flex flex-col gap-1">
                          <div className="w-6 h-6 rounded border border-zinc-700 overflow-hidden hover:border-orange-500 transition-colors shadow-sm" title={getSummonerSpellName(participant.summoner1Id)}>
                          ...
                          <div className="w-6 h-6 rounded border border-zinc-700 overflow-hidden hover:border-orange-500 transition-colors shadow-sm" title={getSummonerSpellName(participant.summoner2Id)}>
                        ...
                        {!arena && participantPrimaryKeystone && (
                          <div className="flex flex-col gap-1 overflow-visible">
                            <div className="w-6 h-6 bg-zinc-800/40 border border-zinc-700 overflow-visible flex items-center justify-center group relative hover:z-9999 hover:border-orange-500 transition-colors shadow-sm" title={getRuneName(participantPrimaryKeystone)}>
                            ...
                            {participantSecondaryTree && (
                              <div className="w-6 h-6 bg-zinc-800/40 border border-zinc-700 overflow-hidden flex items-center justify-center hover:border-orange-500 transition-colors shadow-sm" title={getRuneTreeName(participantSecondaryTree)}>
                      ...
                      <div className="flex-1 min-w-[120px]">
                        ...
                        <div className="flex items-center gap-2">
                          <RankedIcon rankedData={rankedDataMap[participant.puuid]} isLoading={isLoadingRanked} />

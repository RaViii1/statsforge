          ...
          <div className="mb-3 p-2 border-t border-zinc-900">
            ...
            <div className="space-y-1 ">
              {Object.entries(item.stats).filter(([_, value]) => value > 0).map(([stat, value]) => {
                const statTypeMap: Record<string, { type: 'mr' | 'health' | 'mana' | 'armor' | 'dps' | 'dmg' | 'crit' | 'attackspeed' | 'ap' | 'gold' | 'dmgamp' | 'durability' | 'lifesteal', color: string }> = {
                  ...
                  crit_dmg: { type: 'crit', color: 'text-white' },
                  healing: { type: 'health', color: 'text-green-300' },
                  shield: { type: 'armor', color: 'text-white' },
                  lifesteal: { type: 'lifesteal', color: 'text-red-600' },
                  durability: { type: 'durability', color: 'text-green-400' },

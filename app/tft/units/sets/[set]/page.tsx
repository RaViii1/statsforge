            ...
            <div className="absolute bottom-0 left-0 right-0 px-2.5 pb-2.5">
              <div className="flex flex-wrap gap-1">
                {(champion.trait_details ?? []).slice(0, 3).map((trait: any) => (
                  ...
                  >
                    {trait.icon_path && (
                      <img src={getTraitIconUrl(trait.icon_path)} alt={trait.name} className="w-3 h-3 object-contain shrink-0" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                    )}
                    {trait.name}
                  </span>
                ))}
    ...
    <>
      ...
      <div className="min-h-screen bg-zinc-950 text-zinc-100 selection:bg-orange-500/30">
        ...
        <main className="relative max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {activeSets.length > 1 && (
            <div className="flex items-end gap-0 mt-auto border-b border-zinc-900 pb-px">
